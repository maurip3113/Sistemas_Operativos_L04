...
#define INIT "INIT"
 ...
 void fat_fuse_log_activity(char *operation_type, fat_file target_file)
 {
     ...
     fat_tree_node log = fat_tree_node_search(tree, LOG_FILE);
     fat_file log_file = fat_tree_get_file(log);
     fat_file_pwrite(log_file, buf, strlen(buf), log_file->dentry->file_size, fat_tree_get_parent(log));
}
...
static void fat_fuse_log_init(void){  
    fat_volume vol = get_fat_volume();
    fat_tree tree = vol->file_tree;
    if(fat_tree_node_search(tree, LOG_FILE) == NULL){
        mode_t mode =
            S_IFIFO | S_IWUSR | S_IRUSR | S_IRGRP | S_IROTH | S_IWGRP | S_IWOTH; //no sirve para nada

        if (fat_fuse_mknod(LOG_FILE, mode, 0) != 0) {
            DEBUG("Unable to creat " LOG_FILE);
            return;
        }
        
        fat_tree_node log_node = fat_tree_node_search(tree, LOG_FILE);
        ...
        //lo metemos en el arbol (nose si es necesario, no cambia nada)
        /*
        fat_tree_node log_parent_node = fat_tree_node_search(vol->file_tree, "/..");
        fat_tree_insert(vol->file_tree, log_parent_node, log_file);        
        */
        fat_file_hide(log_file,parent);
...
int fat_fuse_readdir(const char *path, void *buf, fuse_fill_dir_t filler,
    ...
    child = children;
    while (*child != NULL) {
        // No agregar /fs.log a la lista, para ocultarlo de ls.
...
int fat_fuse_write(const char *path, const char *buf, size_t size, off_t offset,
    ...
    return fat_file_pwrite(file, buf, size, offset, parent); //intruccion write de bajo nivel (se mete con los cluster) nosotros no ahorramos esto
}
...
int fat_fuse_mknod(const char *path, mode_t mode, dev_t dev) {
    ...
    // The system has already checked the path does not exist. We get the parent
    vol = get_fat_volume();
    parent_node = fat_tree_node_search(vol->file_tree, dirname(strdup(path))); //busca el padre yauqe en el padre estara la directory entry
    if (parent_node == NULL) {
        errno = ENOENT;
        return -errno;
    }
    ...
    fat_file_dentry_add_child(parent, new_file); //mete la dentry en los datos del padre
    return -errno;
}
...
int fat_fuse_unlink(const char *path){
    ...
    parent = fat_tree_get_parent(file_node);
    fat_file_free_cluster(file, parent);  //Mark all the clusters in the file as free
    vol->file_tree = fat_tree_delete(vol->file_tree, path); //Remove the node of the tree
    return -errno;
}
...
int fat_fuse_rmdir(const char *path){
    ...
    GList *children_list = fat_file_read_children(dir); 
    if (children_list != NULL || g_list_length(children_list) != 0) {
        fat_error("Directory not empty"); 
        errno = ENOTEMPTY;  
        return -errno;
    }
    ...
    fat_file_free_cluster(dir, parent);  //Mark all the clusters in the file as free
    vol->file_tree = fat_tree_delete(vol->file_tree, path); //Eliminamos el nodo del arbol
    return -errno;
}
