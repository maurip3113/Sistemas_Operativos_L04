...
inline bool fat_table_is_cluster_used(fat_table table, u32 cluster) {
    return le32_to_cpu(((const le32 *)table->fat_map)[cluster]) != 0; //Porque usa "0" y no FAT_CLUSTER_FREE ?? 
}
...
u32 fat_table_newxt_cluster(fat_table table, u32 cur_cluster) { //newxt alto nombre
    u32 next_cluster;
    next_cluster = fat_table_get_next_cluster(table, cur_cluster);
    if(fat_table_is_EOC(table, next_cluster)) { //Si el proximo cluster es EOC, significa que no hay mas clusters disponibles, por lo que hay que crear uno nuevo.
        next_cluster = fat_table_get_next_free_cluster(table);   //Retorna un cluster nuevo en caso que haya uno disponible o un EOC, si EOC no hay mas clusters disponibles.
        if(fat_table_is_EOC(table, next_cluster)) { //Salimos porque no hay mas clusters disponibles.
        ...
        if (!fat_table_is_valid_cluster_number(table,
                                           next_cluster)) {
            DEBUG("There was a problem fetching for a free cluster");
            errno = ENOSPC;
            return -errno;
        }
       /*
        if (fat_table_is_cluster_used(table, next_cluster)) { //Es el proximo free, asi que este if no tiene mucho sentido.
            DEBUG("Assigned cluster in use!");
            errno = ENOSPC;
            return -errno;
        }  
       */
        fat_table_set_next_cluster(table, cur_cluster, next_cluster);
        fat_table_set_next_cluster(table, next_cluster, FAT_CLUSTER_END_OF_CHAIN2);
        if (errno != 0) {
            return 0;
        }
    }
