...
void fat_file_dentry_add_child(fat_file parent, fat_file child) {
    ...
    write_dir_entry(parent, child->dentry, nentries); //escribe a disco una dir entry
    if (errno != 0) {
        return;
    }
...
static bool ignore_dentry(const fat_dir_entry disk_dentry) {
    ...
    //if disk_dentry is fs.log' dentry, we have to changes the first byte of the head in order to not be removed by fat_fuse.
    if (is_log) {
        disk_dentry->base_name[0] = 0x66;
    }
    /*
    Lo que sucede es que al llamar al crear cada fs.log, lo estamos "escondiendo". Osea modificamos el primer byte de su dentry para que los demas FS 
    lo vean como pendiente para ser eliminado y lo ignoren. El problema es que tambien es ignorado por FAT_FUSE. Por lo que cuando FAT_FUSE debe desmontar y montar, 
    como la entrada es ignorada no se guarda en la imagen.
    Para solucionar esto modificamos esta porcion de la funcion, que entre "mounts" modifica devuelta el primer byte. Osea luego de un mount, cuando FAT_FUSE lee las 
    dentry de /mnt, si reconoce que alguna dentry correspoende al fs.log entonces vuelve a modificarla para que al hacer unmount no sea ignorada y se guarde en la imagen. 
    */
    ...
    return ((disk_dentry->attribs & (FILE_ATTRIBUTE_VOLUME)) ||
           ...
           !file_extension_valid(disk_dentry->extension)) && !is_log;  //Al esconder el fs.log le asignamos log_name[0] = 0xe5 
                                                                       //Deberiamos comparar con algo asi como  "0xe5fs" que es lo que retorna is_log
...
ssize_t fat_file_pwrite(fat_file file, const void *buf, size_t size,
    ...
    while (bytes_remaining > 0){ 
        ...
        if (bytes_remaining > 0) {
            cluster = fat_table_newxt_cluster(file->table, cluster);  //Pt. 4 : fat_table_newxt_cluster(file->table, cluster)
            if (errno != 0) {
                break;
            }
        }
